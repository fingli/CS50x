...
int main(int argc, string argv[])
{
    ...
    while (true)
    {
		...
		// TODO make everything capitalized
...
for (int i = 0; i < WORDS; i++) {	
	// binary search
	search = bsearch(s, dictionary.words[0].letters, WORDS, LETTERS, (strcmp(s, dictionary.words[i].letters)));
...
{
    // TODO --> DONE
    char temp[DIMENSION][DIMENSION]; // make temporary place

    for (int row = 0; row < DIMENSION; row++) // copy letters from grid to temp
    {
        for (int col = 0; col < DIMENSION; col++)
            temp[row][col] =  grid[row][col];
    }
        
    for (int row = 0; row < DIMENSION; row++) // rearrange letters
    {
        for (int col = 0; col < DIMENSION; col++)
        {
            int row1 = (DIMENSION - 1) - col;
            grid[row][col] = temp[row1][row];
        }
    }
